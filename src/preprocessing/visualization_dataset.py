...
DATA_PATH = '/cluster/scratch/wormaniec/old_dsl_data/'
...
def create_mapping_df(patches_df, tracking_df, regions=None, distance=10):
    ...
    for i in tqdm.tqdm(range(num_frames - 1), desc="Creating DataFrame..."):
        ...
        frame2 = frame2.rename(columns={'patch': 'patch' + str(i + 1), 'nr_cells': 'nr_cells' + str(i + 1),
                                        'center_row': 'center_row' + str(i + 1),
                                        'center_col': 'center_col' + str(i + 1)})
        ...
        if mapping is None:
            ...
            frame1 = frame1.rename(columns={'patch': 'patch' + str(i), 'nr_cells': 'nr_cells' + str(i),
                                            'center_row': 'center_row' + str(i), 'center_col': 'center_col' + str(i)})
            columns += ['patch' + str(i), 'nr_cells' + str(i), 'center_row' + str(i), 'center_col' + str(i)]
        ...
        columns += ['patch' + str(i + 1), 'nr_cells' + str(i + 1), 'center_row' + str(i + 1), 'center_col' + str(i + 1)]
        ...
        mapping = mapping[abs(mapping[f"center_row{i + 1}"] - mapping["center_row0"]) < distance]
        mapping = mapping[abs(mapping[f"center_col{i + 1}"] - mapping["center_col0"]) < distance]
...
def turn_to_numpy_and_drop_columns(df):
    """
    ...
    pandas dataframe: Metadata of the patches
    """

    dataset = None
    columns = df.columns
    columns_to_drop = []
    for column in tqdm.tqdm(columns, desc="Turning to numpy array..."):

        if column.startswith('patch'):
            if dataset is None:
                dataset = resize_patch_column(df[column])
            else:
                dataset = np.concatenate((dataset, resize_patch_column(df[column])), axis=1)
...
def main():
    ...
    parser.add_argument('--regions', nargs='+', default=None,
                        help='Regions to consider in image. Format: x1 y1 x2 y2 ...')
    ...
    # turn to numpy array
    mapping, metadata = turn_to_numpy_and_drop_columns(mapping)
    print(metadata.head())
    ...
    metadata.to_csv(DATA_PATH + 'cell_datasets/metadata' + fname + '.csv')
